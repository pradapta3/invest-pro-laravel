    ...
    <div class="p-4 border-b border-slate-100 flex flex-col md:flex-row gap-3 items-center">
        <form method="GET" class="flex items-center gap-2 w-full md:w-64">
            ...
            <div class="relative w-full">
                <x-icon name="search" class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm w-4 h-4" :solid="true" />
        ...
        <div class="flex gap-2 overflow-x-auto w-full">
            <x-filter-chip :href="'?q='.$query.'&f=bsjp'" :active="$filter === 'bsjp'" variant="purple" icon="moon">BSJP</x-filter-chip>
            <x-filter-chip :href="'?q='.$query.'&f=stockpick'" :active="$filter === 'stockpick'" variant="gold" icon="trophy">Top Picks</x-filter-chip>
            <x-filter-chip :href="'?q='.$query.'&f=watchlist'" :active="$filter === 'watchlist'" icon="star">Watchlist</x-filter-chip>
                            ...
                            <div class="flex items-center gap-3">
                                <form method="POST" action="{{ route('dashboard.toggle-watchlist', $cleanTicker) }}">
                                    ...
                                    <button type="submit" class="text-lg {{ $isWatchlisted ? 'text-amber-400' : 'text-slate-200 hover:text-amber-300' }}">
                                        {{-- Filled when watchlisted, outlined when not — the same distinction
                                             fa-solid/fa-regular carried. --}}
                                        <x-icon name="star" class="w-4 h-4" :solid="$isWatchlisted" />
                                    </button>
                                </form>
                                <div>
